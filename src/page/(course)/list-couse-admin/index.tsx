// react
import { Link } from 'react-router-dom'
import { useEffect } from 'react'

// components
import { Button } from '@/components/ui/button'
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table'
import { Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover'

// redux
import { useAppSelector } from '@/hooks/useAppSelector'
import { useAppDispatch } from '@/hooks/useAppDispatch'
import { getCoursesThunk } from '@/redux/course/action'
import { RootState } from '@/redux/store'
import { ICourse } from '@/types/couser'

const ListCourseAdmin = () => {
  const dispatch = useAppDispatch()
  const { courses } = useAppSelector((state: RootState) => state.course)
  useEffect(() => {
    async function fetchCourses() {
      try {
        await dispatch(getCoursesThunk())
      } catch (error) {
        console.error('Error fetching courses:', error)
      }
    }
    fetchCourses()
  }, [dispatch])

  return (
    <div className='mt-10'>
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>Course Name</TableHead>
            <TableHead>Description</TableHead>
            <TableHead>Thubmail</TableHead>
            <TableHead>Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {courses &&
            courses.map((course: ICourse) => (
              <TableRow key={course?._id}>
                <TableCell>{course?.name}</TableCell>
                <TableCell>{course?.description}</TableCell>
                <TableCell>
                  <img src={course?.image as string} alt={course?.name} className='w-32 h-20 object-cover' />
                </TableCell>
                <TableCell>
                  <div className='flex gap-x-4'>
                    <Button variant={'destructive'} className='p-2'>
                      <svg
                        xmlns='http://www.w3.org/2000/svg'
                        fill='none'
                        viewBox='0 0 24 24'
                        strokeWidth={1.5}
                        stroke='currentColor'
                        className='size-4'
                      >
                        <path
                          strokeLinecap='round'
                          strokeLinejoin='round'
                          d='m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0'
                        />
                      </svg>
                    </Button>
                    <Popover>
                      <PopoverTrigger className='p-2'>
                        <svg
                          xmlns='http://www.w3.org/2000/svg'
                          fill='none'
                          viewBox='0 0 24 24'
                          strokeWidth={1.5}
                          stroke='currentColor'
                          className='size-4'
                        >
                          <path
                            strokeLinecap='round'
                            strokeLinejoin='round'
                            d='m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10'
                          />
                        </svg>
                      </PopoverTrigger>
                      <PopoverContent className='max-w-[100px] w-full py-3'>
                        <div className='flex flex-col gap-y-2 text-center'>
                          <Link className='hover:underline ' to={`/course/edit/${course._id}`}>
                            Course
                          </Link>
                          <div className='w-full border-inherit border-b'></div>
                          <Link className='hover:underline' to={`/course/${course._id}/chapter`}>
                            Chapter
                          </Link>
                        </div>
                      </PopoverContent>
                    </Popover>
                  </div>
                </TableCell>
              </TableRow>
            ))}
        </TableBody>
      </Table>
    </div>
  )
}

export default ListCourseAdmin
